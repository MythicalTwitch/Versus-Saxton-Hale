...
{
	name = "VSH Damage Modifiers",
	author = "VSH Team",
	description = "Track Saxton's Damage Modifiers",
	version = VERSION,
	url = "https://forums.alliedmods.net/showthread.php?t=146884"
}

public OnVSH_TakeDmgAllPre(victim,attacker,&Float:damage,damagecustom)
{
	new Hale = GetClientOfUserId(VSH_GetSaxtonHaleUserId);
	if (VSHRoundState == ROUNDSTATE_EVENT_ROUND_START && (victim == Hale || (victim != attacker && attacker != Hale)))
	{
		VSH_DamageModPercent(0.0);
		return;
	}
}

public OnVSH_TakeDmgAll(victim,attacker,Float:damage)
{
	new Hale = GetClientOfUserId(VSH_GetSaxtonHaleUserId);
	if (!VSH_IsSaxtonHaleModeEnabled() || !IsValidEdict(attacker) || ((attacker <= 0) && (victim == Hale)) || TF2_IsPlayerInCondition(victim, TFCond_Ubercharged))
		return Plugin_Continue;

	decl Float:Pos[3];
	GetEntPropVector(attacker, Prop_Send, "m_vecOrigin", Pos);
	if ((attacker == Hale) && IsValidClient(victim) && (victim != Hale) && !TF2_IsPlayerInCondition(victim, TFCond_Bonked) && !TF2_IsPlayerInCondition(victim, TFCond_Ubercharged))
	{
		...
		{
			if (GetEntPropEnt(ent, Prop_Send, "m_hOwnerEntity") == victim && !GetEntProp(ent, Prop_Send, "m_bDisguiseWearable"))
			{
...
//          return Plugin_Changed;
		...
		new buffweapon = GetPlayerWeaponSlot(victim, TFWeaponSlot_Secondary);
		new buffindex = (IsValidEntity(buffweapon) && buffweapon > MaxClients ? GetEntProp(buffweapon, Prop_Send, "m_iItemDefinitionIndex") : -1);
		if (buffindex == 226)
		{
			CreateTimer(0.25, Timer_CheckBuffRage, GetClientUserId(victim), TIMER_FLAG_NO_MAPCHANGE);
		}
		if (damage <= 160.0
	...
	else if (attacker != Hale && victim == Hale)
	{
				...
				switch (wepindex)
				{
					...
					case 416:   // Chdata's Market Gardener backstab
					{
						if (GetRJFlag(attacker))
						{
							...
							//if ((GetEntProp(victim, Prop_Send, "m_iStunFlags") & TF_STUNFLAGS_GHOSTSCARE | TF_STUNFLAG_NOSOUNDOREFFECT) && Special == VSHSpecial_HHH) return Plugin_Continue;
					...
					/*case 528:
					{
						if (circuitStun > 0.0)
						{
							TF2_StunPlayer(victim, circuitStun, 0.0, TF_STUNFLAGS_SMALLBONK|TF_STUNFLAG_NOSOUNDOREFFECT, attacker);
							EmitSoundToAll("weapons/barret_arm_zap.wav", victim);
							EmitSoundToClient(victim, "weapons/barret_arm_zap.wav");
						}
					}*/
					case 656:
					{
						CreateTimer(0.1, Timer_StopTickle, GetClientUserId(victim), TIMER_FLAG_NO_MAPCHANGE);
...
/*              if ( IsValidEdict( weapon ) && GetEdictClassname( weapon, wepclassname, sizeof( wepclassname ) ) && strcmp( wepclassname, "tf_weapon_stickbomb", false ) == 0 )
				...
				if (bIsBackstab)
				{
					...
					*/
					...
					EmitSoundToClient(victim, "player/spy_shield_break.wav", _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, 0.7, 100, _, Pos, NULL_VECTOR, false, 0.0);
					EmitSoundToClient(attacker, "player/spy_shield_break.wav", _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, 0.7, 100, _, Pos, NULL_VECTOR, false, 0.0);
					EmitSoundToClient(victim, "player/crit_received3.wav", _, _, SNDLEVEL_TRAFFIC, SND_NOFLAGS, 0.7, 100, _, _, NULL_VECTOR, false, 0.0);
					...
					PrintCenterText(victim, "You were just backstabbed!");
					/*new Handle:stabevent = CreateEvent("player_hurt", true);
					SetEventInt(stabevent, "userid", GetClientUserId(victim));
			...
			{
				if (wepindex == 45 || ((wepindex == 209 || wepindex == 294 || wepindex == 23 || wepindex == 160 || wepindex == 449) && (TF2_IsPlayerCritBuffed(victim) || TF2_IsPlayerInCondition(victim, TFCond_CritCola) || TF2_IsPlayerInCondition(victim, TFCond_Buffed) || TF2_IsPlayerInCondition(victim, TFCond_CritHype))))
				{
	...
	else if (attacker == 0 && victim != Hale && IsValidClient(victim, false) && (damagetype & DMG_FALL) && (TF2_GetPlayerClass(victim) == TFClass_Soldier || TF2_GetPlayerClass(victim) == TFClass_DemoMan))
	{
		new item = GetPlayerWeaponSlot(victim, (TF2_GetPlayerClass(victim) == TFClass_DemoMan ? TFWeaponSlot_Primary:TFWeaponSlot_Secondary));
